...
export function LegalPageShell({ label, title, intro, updated, children }: LegalPageShellProps) {
  return (
    <main className="min-h-screen bg-cream/35 transition-colors dark:bg-[#0e1012]">
      <SiteHeader />
      ...
      <section className="mx-auto max-w-5xl px-5 py-12 md:px-8 md:py-16">
        ...
        <h1 className="mt-3 text-3xl font-bold leading-tight text-ink dark:text-white md:text-4xl">{title}</h1>
        <div className="mt-4 max-w-3xl text-sm leading-7 text-ink/70 dark:text-white/60 md:text-base">{intro}</div>
        {updated ? (
          <p className="mt-3 text-sm font-medium text-ink/55 dark:text-white/45">{updated}</p>
        ) : null}
...
export function LegalSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="rounded-2xl border border-pine/10 bg-white p-5 shadow-soft transition-colors dark:border-white/10 dark:bg-[#181b1d] md:p-6">
      <h2 className="text-xl font-bold text-ink dark:text-white">{title}</h2>
      <div className="mt-3 space-y-3 text-sm leading-7 text-ink/72 dark:text-white/62 md:text-base">{children}</div>
    </section>
  );
}
...
export function LegalContactBox({
  ...
  return (
    <section className="rounded-2xl border border-pine/20 bg-mint/35 p-5 shadow-soft transition-colors dark:border-pine/20 dark:bg-pine/10 md:p-6">
      ...
      <div className="mt-2 text-sm leading-7 text-ink/70 dark:text-white/60 md:text-base">
        {children ?? (
          <p>
