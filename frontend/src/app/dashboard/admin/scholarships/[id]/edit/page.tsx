...
function AdminScholarshipEditContent() {
  ...
  async function handleApplyDetectedDeadline(dateValue: string, evidence = "", sourceUrl = "") {
    ...
    try {
      ...
      setDeadlineMessage("Detected deadline applied. Social caption refreshed and uploaded image marked stale.");
    } catch (requestError) {
                  ...
                  <CardContent className="grid gap-3 p-3 md:p-4">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-ink/60 dark:text-white/50">
                        Prepare evidence for GPT review, then apply only a verified detected
                        deadline.
                      </p>
                    </div>
                    ...
                    <div className="grid gap-1.5 text-xs leading-5 text-ink/70 dark:text-white/60">
                      ...
                      <div>
                        Last checked: {opportunity.deadline_last_checked_at || "Never"}
                      </div>
                      <div>Status: {opportunity.deadline_check_status || "unchecked"}</div>
                      <div>
                        Confidence: {opportunity.deadline_check_confidence || "Not set"}
                      </div>
                      ...
                      {opportunity.social_image?.image_is_stale ? (
                        <div className="rounded-xl border border-saffron/30 bg-saffron/10 px-3 py-2 font-semibold text-ink dark:text-white">
                          Deadline changed. Uploaded social image may contain old deadline.
                          Upload a new image or use OG fallback.
                        </div>
                      ) : null}
                    </div>
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => void handlePrepareDeadlineCheck()}
                        ...
                        {deadlinePackage.candidate_dates.length > 0 ? (
                          deadlinePackage.candidate_dates.slice(0, 4).map((candidate) => (
                            <div key={`${candidate.date}-${candidate.evidence}`} className="grid gap-1">
                              ...
                              <div className="text-ink/65 dark:text-white/60">
                                {candidate.evidence}
                              </div>
                              <Button
                                type="button"
                                variant="outline"
                                onClick={() =>
                                  void handleApplyDetectedDeadline(
                                    candidate.date,
                                    candidate.evidence,
