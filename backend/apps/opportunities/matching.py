...
def _has_open_country_eligibility(values):
    open_values = {
        "all",
        "all countries",
        "any",
    ...
    return bool(set(values) & open_values)


def _has_open_field_eligibility(values):
    open_values = {
        "all",
        "all fields",
        "any",
...
def _related_names(instance, related_name):
    ...
    try:
        return [str(name).strip() for name in manager.values_list("name", flat=True) if str(name).strip()]
    except Exception:
        return []


def _apply_hard_mismatch_cap(score, profile, opportunity, missing_requirements, warnings):
    blockers = 0

    degree_levels = _normalized_list(getattr(opportunity, "degree_levels", []))
    target_degree = _normalized_text(getattr(profile, "target_degree_level", ""))

    if degree_levels and target_degree and target_degree not in degree_levels:
        blockers += 1
        add_unique(missing_requirements, "Target degree level does not match this opportunity.")

    opportunity_fields = _normalized_list(getattr(opportunity, "fields_of_study", []))
    opportunity_fields.extend(_normalized_list(_related_names(opportunity, "study_field_refs")))
    opportunity_fields = sorted(set(opportunity_fields))

    profile_fields = set(_normalized_list(getattr(profile, "target_fields", [])))
    profile_fields.update(_normalized_list(_related_names(profile, "target_study_field_refs")))

    current_field = _normalized_text(getattr(profile, "current_field_of_study", ""))
    if current_field:
        profile_fields.add(current_field)

    current_field_ref = getattr(profile, "current_study_field_ref", None)
    if current_field_ref:
        profile_fields.add(_normalized_text(current_field_ref.name))

    custom_current_field = _normalized_text(getattr(profile, "custom_current_study_field", ""))
    if custom_current_field:
        profile_fields.add(custom_current_field)

    custom_target_fields = _normalized_list(getattr(profile, "custom_target_study_fields", []))
    profile_fields.update(custom_target_fields)

    if (
        opportunity_fields
        and not _has_open_field_eligibility(opportunity_fields)
        and profile_fields
        and profile_fields.isdisjoint(set(opportunity_fields))
    ):
        blockers += 1
        add_unique(missing_requirements, "Field of study does not match this opportunity.")

    eligible_countries = _normalized_list(getattr(opportunity, "eligible_countries", []))
    eligible_countries.extend(_normalized_list(_related_names(opportunity, "eligible_country_refs")))
    eligible_countries = sorted(set(eligible_countries))

    nationality = _normalized_text(getattr(profile, "nationality", ""))
    nationality_country = getattr(profile, "nationality_country", None)
    if nationality_country:
        nationality = _normalized_text(nationality_country.name)

    if (
        eligible_countries
        and not _has_open_country_eligibility(eligible_countries)
        and nationality
        and nationality not in eligible_countries
    ):
        blockers += 1
        add_unique(missing_requirements, "Nationality may not match this opportunity's eligibility.")

    opportunity_country = _normalized_text(getattr(opportunity, "country", ""))
    country_ref = getattr(opportunity, "country_ref", None)
    if country_ref:
        opportunity_country = _normalized_text(country_ref.name)

    target_countries = set(_normalized_list(getattr(profile, "target_countries", [])))
    target_countries.update(_normalized_list(_related_names(profile, "target_country_refs")))

    if opportunity_country and target_countries and opportunity_country not in target_countries:
        add_unique(warnings, "Opportunity country is outside the student's target countries.")
...
def _apply_breakdown_mismatch_cap(score, breakdown, missing_requirements, warnings):
    major_mismatches = 0

    if breakdown.get("degree_level", 0) <= 0:
        major_mismatches += 1

    if breakdown.get("field_fit", 0) <= 0:
        major_mismatches += 1

    if breakdown.get("eligibility", 0) <= 0:
        major_mismatches += 1

    if major_mismatches >= 2:
        add_unique(
            warnings,
            "Major eligibility mismatches limit this match score.",
        )
        return min(score, 40)

    if major_mismatches == 1:
        return min(score, 60)

    return score


def _apply_missing_requirement_mismatch_cap(score, missing_requirements, warnings):
    ...
    if any("degree level" in item and "does not match" in item for item in normalized_requirements):
        blockers += 1

    if any("field" in item and "does not match" in item for item in normalized_requirements):
        blockers += 1

    if any(
        ("nationality" in item or "eligibility" in item or "eligible" in item)
        and ("not match" in item or "may not" in item or "not eligible" in item)
        for item in normalized_requirements
    ):
        blockers += 1

    if blockers >= 2:
        add_unique(warnings, "Major eligibility mismatches limit this match score.")
        return min(score, 40)
...
def calculate_opportunity_match(profile, opportunity):
    ...
    score = _apply_hard_mismatch_cap(
        score,
        profile,
        opportunity,
        missing_requirements,
    ...
    score = _apply_breakdown_mismatch_cap(
        score,
        breakdown,
        missing_requirements,
        warnings,
    ...
    score = _apply_missing_requirement_mismatch_cap(
        score,
        missing_requirements,
        warnings,
    )
